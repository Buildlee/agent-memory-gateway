...
Initial registration uses a one-time pairing code + device public key to complete binding. A code may carry an administrator-defined workspace and capability set; Gateway verifies the workspace owner and active state again before creating the Agent binding. The device proves private key possession with **Ed25519**; the Gateway issues short-term access tokens, with refresh credentials stored only in OS-protected local storage. The pairing code never appears in the command line, PowerShell history, or MCP JSON. When a device or Agent is revoked, the corresponding epoch is incremented, immediately invalidating old tokens and old sync state.
