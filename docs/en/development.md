...
- `memory-admin-console` listens on loopback by default. A central deployment may be proxied by Caddy only when it explicitly enables `--allow-network`, `--secure-cookie`, a fixed `--mount-path /admin`, and a protected launch-link file; binding directly to an arbitrary LAN address remains rejected. The initial URL contains a one-time `session` token that is invalidated after being exchanged for an HttpOnly Cookie (`consume_launch_token` marks it as used). Page source code, API responses, logs, and test assertions do not contain local keys, Gateway tokens, or refresh credentials.
