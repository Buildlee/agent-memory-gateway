...
Windows 的 Sidecar 启动脚本会优先从当前发布副本的 `src` 目录加载代码。这样更新同一份受控源码并重启 Sidecar 时，不需要先替换可能正被 Codex 或 Hermes MCP 客户端占用的 `.exe` 启动文件。发布副本必须保持在已验证的版本；不要把日常试验目录直接设为计划任务的工作目录。若项目目录没有 `src`，脚本会继续使用已安装包。
