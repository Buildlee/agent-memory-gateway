...
不带 `-Apply` 时，脚本只识别容器、Compose 项目、Gateway 和最终 MCP 地址，不会创建目录、配对设备、改数据库或启动容器。配对完成后中断时，使用相同参数加 `-Resume`；它只复用已有的受保护文件，不会重置设备身份。
...
保存后，先调用 `memory_sync_status`。安装器已经验证过 Bridge 到 Gateway 的只读同步和 MCP 初始化；这一步确认 Agent 的管理界面已实际加载新服务器。
