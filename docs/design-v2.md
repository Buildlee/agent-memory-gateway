...
- 概览页显示当前工作区的待审核数量、待重试和死信数量、已授权设备数量，以及最近一次审计时间。它不返回记忆正文。
- 设备页显示设备、Agent、工作区绑定、能力、状态和授权 epoch。不会显示设备私钥、刷新凭据、公钥原文或凭据哈希。
- 审核与冲突页复用现有审核接口。页面先显示候选和冲突来源；执行确认、取代、保留双方、拒绝或撤销前，必须由用户在页面上明确确认。每次操作仍使用 revision 和 idempotency key。
- 运行页显示近期审计、未处理死信的错误码和恢复检查结果。恢复检查只读；它不自动重放事件、清理 outbox 或改写数据库。
