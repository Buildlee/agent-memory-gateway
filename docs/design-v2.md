...
各 Agent 原有的记忆系统继续留在端侧，Gateway 不读取它们的数据库，也不把中枢结果反向写回。端侧 Provider 把不同来源转换成统一记录；内置实现支持 Markdown、JSON 和 JSONL，第三方系统通过插件入口接入。

用户可以逐条选择，也可以让系统从用户偏好、项目决定、稳定事实和长期约定四类内容中自动提议。未选择、含敏感信息、带命令式内容或过长的记录停留在本机。中枢只接收不含路径的来源标识，并按用户、Provider 实例、来源记录和来源版本去重。
...
初次接入用一次性配对码 + 设备公钥完成绑定。配对码可含管理员预先限定的工作区和能力；Gateway 会再次核对工作区归属与状态，再为新 Agent 创建绑定。设备以 **Ed25519** 证明私钥持有；Gateway 发放短期访问令牌，刷新凭据仅保存在受操作系统保护的本机位置。配对码不进入命令行、PowerShell 历史或 MCP JSON。撤销设备或 Agent 时递增对应 epoch，旧令牌和旧同步状态立即失效。
...
每次上下文召回还会生成 `recall_id`。中枢只保存查询摘要、命中引用、设备和 Agent，不保存原始查询。`useful`、`pin`、`outdated` 和 `incorrect` 反馈只提供有上限的排序调整，不能绕过授权，也不会直接改写记忆生命周期。
...
- 本地队列不保存明文敏感记忆。
- Sidecar 把扫描出的 `instruction_like` 标记和规则编号与加密事件一起保存。离线检索会跳过这类记录；即使缓存或队列里出现异常数据，也不能把命令式文本带入 Agent 上下文。
- 不因临时断网改走未保护公网地址。
- 清理已同步密文需用户明确确认。
- 两个 Sidecar 进程不能同时拥有同一个 outbox。

同步下发墓碑时，Gateway 会按记忆的工作区、设备或 Agent 作用域再次过滤。墓碑被撤销后保留审计记录，并以新的生命周期版本把恢复后的记忆重新下发；不会通过物理删除掩盖历史。
...
默认容器布局是 `memory-app + Caddy`：前者监管 Gateway、Worker、管理 Sidecar 和管理页，后者是唯一 HTTPS 入口。需要更小故障域时可使用拆分布局，把逻辑进程放到独立容器；两种布局使用同一协议、数据库和权限模型。数据库只在内部网络可达。
...
```mermaid
flowchart LR
  B["已授权浏览器"] -->|"回环或 HTTPS /admin + 签名会话"| C["管理控制台"]
  C -->|"受 Sidecar key 保护"| S["Memory Sidecar"]
  S -->|"HTTPS + 短期令牌"| G["Memory Gateway"]
  G --> M[("元数据与审计库")]
```

本机管理桥只监听 **127.0.0.1**。中枢控制台只有在显式启用受控网络监听、固定 `/admin` 路径和 `Secure` Cookie 时才允许由 Caddy 代理；它没有宿主机端口。新浏览器先用一次性链接换取带签名和到期时间的 `HttpOnly`、`SameSite=Strict` Cookie。中枢状态目录保存所有者专用的会话签名密钥，因此授权可跨控制台重启保留；一次性链接只写入受保护文件，不出现在页面源码或 Docker 日志。管理桥不读取数据库，也不保存 Gateway 凭据。具体操作见[中枢管理页](central-admin.md)。
...
- **设备与权限**：显示设备、Agent、工作区绑定、能力、状态、授权 epoch。管理者可以修改当前工作区的正式能力，或撤销 Agent、设备和关联刷新凭据；当前管理端不能撤销自身或移除自身的 `memory.manage`。撤销只改变身份状态，不删除记录。不显示设备私钥、刷新凭据、公钥原文或凭据哈希。
- **审核与冲突**：复用审核接口。先显示候选和冲突来源；执行确认、取代、保留双方、拒绝、撤销前需用户明确确认。每次操作使用 `revision` + `idempotency_key`。
- **运行**：待重试、未处理死信的错误码和恢复检查结果。只读。
- **活动**：近期审计。通过受租户和工作区约束的关联查询补充来源设备名、设备类型、Agent 名、Agent 类型和状态；技术 ID 作为折叠详情，不返回审计详情正文。

Gateway 管理接口统一要求 `memory.manage`，按调用者的租户、用户、工作区过滤。只返回结构化元数据（状态、计数、时间、ID、错误码）。候选正文由既有审核服务按授权解密；审计和运行接口不返回正文、密文、令牌、连接串或异常堆栈。

管理入口的概览、检索、运行和活动保持只读。改变记忆状态的请求沿用审核接口，权限修改与身份撤销使用独立管理接口；所有写操作都要求 `confirmed_by_user=true`、期望 revision、授权 epoch 或期望能力集，以及一次性幂等键。管理目标使用 `target_agent_installation_id` / `target_device_id`，避免与调用者身份字段混淆。删除、清理、批量重放不放进控制台默认操作范围。
