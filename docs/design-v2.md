...
`scripts/setup-shared-memory.ps1` 把常用安装动作分成四种模式：`demo` 用于本地体验，`server` 用于发布已经准备好的服务端版本，`device` 用于给一台 Windows 设备配对并接入 Sidecar，`verify` 只检查本机 Sidecar 是否在运行。这样第一次接入不再需要手工拼接密钥、计划任务和 MCP 参数。

`device` 模式从隐藏输入读取一次性配对码，创建设备私钥和 Sidecar key，并把刷新凭据交给 Windows Credential Manager。它默认建立独立的 Python 运行环境，启动后只生成不含凭据的 MCP JSON。已有私钥、key、计划任务或 MCP 配置时会停止，不会覆盖。配对已成功但本机后续步骤被打断时，用户可显式传入 `-UseExistingCredential` 继续；脚本只复用受保护存储中的现有凭据，不会读取、替换或输出它。

`server` 模式在没有 `-Apply` 时只回显经过校验的发布信息。加上 `-Apply` 才会创建新的发布目录、上传公开代码并启动容器。密钥、证书、数据库迁移和备份不属于自动化范围：这些动作需要管理员在维护窗口确认，避免一个方便的命令误改所有设备共用的状态。
...
设备接入分成两层。协议层固定处理配对、设备私钥、刷新凭据轮换、工作区绑定、离线队列和 MCP 工具；这些行为不因设备品牌或 Agent 厂商变化。运行层只解决本地的凭据保存和进程生命周期：Windows 使用 Credential Manager 与计划任务，容器使用权限为 `0600` 的状态目录和一个回环 MCP Bridge。

容器 Bridge 与目标服务共用网络命名空间，标准地址固定为 `http://127.0.0.1:8767/mcp`。因此需要适配的只是某个 Agent 的官方 MCP 设置入口，而不是重新实现一套记忆服务。配置连接器不能保存刷新凭据、直接操作数据库或扩大工作区权限。
...
首次接入使用一次性配对码和设备公钥完成绑定。设备以 Ed25519 证明持有私钥；Gateway 发放短期访问令牌，刷新凭据仅保存在受操作系统保护的本机位置。配对码不进入命令行、PowerShell 历史或 MCP JSON。撤销设备或 Agent 时递增对应 epoch，使旧令牌和旧同步状态失效。
...
管理控制台只提供给拥有 `memory.manage` 的已登记 Agent。浏览器不直接保存 Gateway 令牌、刷新凭据或数据库连接串；它连接的是本机管理桥，桥再通过现有 Sidecar 转发请求。这样浏览器页面、MCP 和自动化脚本使用同一套设备身份和工作区授权。

```mermaid
flowchart LR
  B["浏览器"] -->|"127.0.0.1 + 一次性会话"| C["本机管理桥"]
  C -->|"受本机 key 保护"| S["Memory Sidecar"]
  S -->|"HTTPS + 短期令牌"| G["Memory Gateway"]
  G --> M[("元数据与审计库")]
```

本机管理桥只监听 `127.0.0.1`。启动时生成一次性随机会话，首个浏览器请求换取仅本机会话 Cookie；会话值不写入文件，不出现在页面源码和日志中。管理桥不读取数据库，也不保存 Gateway 凭据。它只保留本机 Sidecar 必需的 outbox key，并由 Sidecar 决定哪个 Agent 身份可以取得短期访问令牌。

当前本机实现由 `memory-admin-console` 启动，Windows 可通过 `scripts/start-admin-console.ps1` 读取 Sidecar key 文件并传入默认工作区。脚本会清掉继承来的 Gateway 令牌、刷新凭据和设备配置，只保留连接本机 Sidecar 所需的最小环境变量。

管理页、Sidecar 和 Gateway 必须运行包含同一组管理方法的发布版本。升级后如果页面提示本机 Sidecar 尚不支持该方法，应先完成受控更新，再在维护窗口重启这一台设备的 Sidecar 并运行只读健康检查；不要为了让页面可用而让浏览器绕过 Sidecar 直连 Gateway。

管理页面分为六块：

- 概览页显示当前工作区的待审核、待重试、死信和活跃设备数量，并把需要处理的事项、健康检查和近期活动放在同一处。Sidecar 尚未更新或 Gateway 不可用时，页面必须给出可理解的恢复提示，不能只留下骨架屏。
- 记忆页只通过现有 Sidecar 调用当前 Agent 的授权检索能力。用户输入关键词后，页面才显示当前工作区内已授权的记忆内容、来源类型、状态和置信度；它不提供绕过审核的新建、删除或批量改写入口。
- 设备与权限页显示设备、Agent、工作区绑定、能力、状态和授权 epoch。不会显示设备私钥、刷新凭据、公钥原文或凭据哈希。
- 审核与冲突页复用现有审核接口。页面先显示候选和冲突来源；执行确认、取代、保留双方、拒绝或撤销前，必须由用户在页面上明确确认。每次操作仍使用 revision 和 idempotency key。
- 运行页显示待重试、未处理死信的错误码和恢复检查结果；活动页单独显示近期审计。两页都只读，不自动重放事件、清理 outbox 或改写数据库。

Gateway 的管理数据接口统一要求 `memory.manage`，并按照调用者的租户、用户和工作区过滤。接口只返回页面需要的结构化元数据：状态、计数、时间、稳定 ID 和错误码。候选正文继续由既有审核服务按授权解密；审计和运行接口不返回正文、密文、令牌、连接串或异常堆栈。

管理入口默认只读。会改变记忆状态的请求沿用审核接口，并要求页面携带 `confirmed_by_user=true`、期望 revision 和一次性幂等键。删除、清理和批量重放不放进控制台的默认操作范围；这类动作仍需要单独的受控流程和用户确认。
