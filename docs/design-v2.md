...
管理控制台只提供给拥有 `memory.manage` 的已登记 Agent。浏览器不直接保存 Gateway 令牌、刷新凭据或数据库连接串。它连接本机管理桥，桥通过 Sidecar 转发请求。浏览器页面、MCP 和自动化脚本共用同一套设备身份和工作区授权。
...
本机管理桥只监听 **127.0.0.1**（`admin_console.py:1405`）。启动时生成一次性随机会话，首个浏览器请求换取仅本机会话 Cookie；会话值不写入文件，不出现在页面源码和日志中。管理桥不读取数据库，也不保存 Gateway 凭据。
